...
export default function FloatingChat({ lang }: FloatingChatProps) {
  ...
  // Hide when immersive mode is active
  useEffect(() => {
  ...
  // Emit chatToggle event for ambient music ducking (if any)
  useEffect(() => {
  ...
  return (
    <>
      ...
      <motion.button
        ...
        aria-label={lang === 'en' ? (isOpen ? 'Close chat' : 'Open chat') : (isOpen ? 'Cerrar chat' : 'Abrir chat')}
      >
